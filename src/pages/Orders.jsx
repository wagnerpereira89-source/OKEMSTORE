...
function OrderCard({ order, tipo, selected, onToggleSelect, onChangeStatus, onPrint, processing }) {
  ...
  const typePill = {
    retirada: { icon: '🛍️', label: 'Retirada', bg: '#3A2E12', color: '#E8C878' },
    envio:    { icon: '📦', label: 'Envio',    bg: '#12233A', color: '#8AB4E8' },
    motoboy:  { icon: '🛵', label: 'Motoboy',  bg: '#3A1226', color: '#E890B8' },
  }[orderType]
    ...
    <div className="bg-[#1A1A1A] border border-[#2E2E2E] rounded-2xl p-4">
      ...
      <div className="flex items-center justify-between mb-3">
        <label className="flex items-center gap-2 cursor-pointer">
          ...
          />
          <span className="text-sm font-bold text-[#F5F5F0]">#{order.id}</span>
        </label>
        {status && (
          <span
      ...
      {/* Cliente */}
      <div className="mb-3">
        <p className="text-base font-semibold text-[#F5F5F0]">{cliente}</p>
        {phoneClean && (
          <a
      ...
      <div className="space-y-1.5 mb-3 text-sm">
        <div className="flex justify-between">
          <span className="text-[#C4C4C4]">Tipo</span>
          <span
        ...
        <div className="flex justify-between">
          <span className="text-[#C4C4C4]">Itens</span>
          <span className="text-[#F5F5F0]">{qtdTotal} {qtdTotal === 1 ? 'item' : 'itens'}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-[#C4C4C4]">Total</span>
          <span className="font-semibold text-[#F5F5F0]">{formatBRL(order.total)}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-[#C4C4C4]">Pagamento</span>
          <span className="text-[#F5F5F0] text-xs text-right max-w-[60%]">{payment}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-[#C4C4C4]">{orderType === 'retirada' ? 'Local' : 'Entrega'}</span>
          <span className="text-[#F5F5F0] text-xs text-right max-w-[60%]">{shippingLabel}</span>
        </div>
        <div className="flex justify-between">
          <span className="text-[#C4C4C4]">Data</span>
          <span className="text-[#F5F5F0] text-xs">{formatDate(order.date_created)}</span>
        </div>
      </div>
                ...
                return (
                  <div key={item.id} className="flex gap-3 items-start">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium text-[#F5F5F0] leading-snug">{item.name}</p>
                      {meta.map((m, i) => (
                        <p key={i} className="text-xs text-[#C4C4C4] mt-0.5">
                          {m.display_key || m.key}: <strong>{m.display_value || m.value}</strong>
                        </p>
                      ))}
                      <p className="text-xs text-[#C4C4C4] mt-0.5">Qtd: <strong>{item.quantity}</strong></p>
                    </div>
                  </div>
                )
              })}
      ...
      {/* Ações */}
      <div className="flex flex-col gap-2 pt-3 border-t border-[#242424]">
        {next && (
          <button
            onClick={() => onChangeStatus(order.id, next.key)}
            disabled={processing}
            className="w-full h-10 bg-primary text-[#141414] rounded-xl text-sm font-medium hover:bg-[#CFCFCF] disabled:opacity-50 flex items-center justify-center gap-1"
          >
        ...
        {canPrint && (
          <button
            ...
            className="w-full h-10 bg-[#1A1A1A] border border-primary text-primary rounded-xl text-sm font-medium hover:bg-primary/5 flex items-center justify-center gap-1"
          >
        ...
        {back && (
          <button
            ...
            disabled={processing}
            className="w-full h-9 border border-[#2E2E2E] text-[#C4C4C4] rounded-xl text-sm hover:bg-[#242424] disabled:opacity-50"
          >
            ← {back.label}
          </button>
        )}
  ...
  return (
    <div className="min-h-screen bg-[#0D0D0D]">
      ...
      <header className="sticky top-0 z-40 bg-[#1A1A1A] border-b border-[#2E2E2E] px-4 h-20 flex items-center gap-3">
        ...
        <h1 className="text-base font-bold text-[#F5F5F0]">Pedidos</h1>
      ...
      <div className="max-w-mobile mx-auto px-4 pb-32 pt-3">
        {/* Tabs de tipo */}
        <div className="grid grid-cols-4 gap-1 mb-3 bg-[#1A1A1A] border border-[#2E2E2E] rounded-xl p-1">
          {TIPOS.map((t) => {
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center justify-center gap-0.5 py-2 rounded-lg text-[11px] font-medium transition-colors ${
                  active ? 'bg-primary text-[#141414]' : 'text-[#C4C4C4] hover:bg-[#242424]'
                }`}
              >
                <t.Icon size={16} />
                {t.label}
        ...
        <div className="relative mb-3">
          ...
          <input
            ...
            className="w-full h-10 pl-9 pr-9 text-sm bg-[#1A1A1A] border border-[#2E2E2E] rounded-xl focus:outline-none focus:border-primary placeholder:text-[#9A9A9A]"
          />
          {search && (
            ...
            return (
              <button
                ...
                className={`flex-shrink-0 px-3 h-9 rounded-full text-xs font-medium flex items-center gap-1.5 transition-colors ${
                  active ? 'text-white' : 'bg-[#1A1A1A] border border-[#2E2E2E] text-[#C4C4C4]'
                }`}
                style={active ? { background: s.color } : {}}
              >
                {s.label}
                <span className={`px-1.5 py-0.5 rounded-full text-[10px] font-bold ${
                  active ? 'bg-[#1A1A1A]/20 text-white' : 'bg-[#242424] text-[#C4C4C4]'
                }`}>
                  {count}
                </span>
              </button>
        ...
        {loading ? (
          <div className="flex flex-col items-center py-12 gap-3">
            ...
            <p className="text-sm text-[#9A9A9A]">Carregando pedidos...</p>
          </div>
        ) : filteredOrders.length === 0 ? (
          <div className="bg-[#1A1A1A] rounded-2xl border border-[#2E2E2E] p-8 text-center">
            <p className="text-sm text-[#9A9A9A]">Nenhum pedido encontrado</p>
          </div>
        ) : (
          <>
      ...
      {selected.size > 0 && (
        <div className="fixed bottom-16 left-0 right-0 z-30 bg-[#1A1A1A] border-t border-[#2E2E2E] p-3 shadow-lg max-w-mobile mx-auto">
          <div className="flex items-center gap-2 mb-2">
            ...
            <button onClick={clearSelection}
              className="text-xs text-[#9A9A9A] hover:text-[#C4C4C4]">Limpar</button>
          </div>

          {/* Imprimir selecionados */}
          <button
            onClick={() => handlePrint([...selected])}
            className="w-full h-10 mb-2 bg-[#1A1A1A] border border-primary text-primary rounded-xl text-sm font-medium flex items-center justify-center gap-1"
          >
          ...
          <div className="flex gap-2">
            <select
              ...
              className="flex-1 h-10 px-3 text-sm bg-[#1A1A1A] border border-[#2E2E2E] rounded-xl focus:outline-none focus:border-primary"
            >
            ...
            <button
              ...
              className="h-10 px-4 bg-primary text-[#141414] rounded-xl text-sm font-medium disabled:opacity-50"
            >
