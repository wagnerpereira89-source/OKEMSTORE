...
function AttributeBlock({ attribute, selectedValues, onToggleValue, onAddCustomValue, customAttrCreation }) {
  ...
  return (
    <div className={`bg-[#1A1A1A] border rounded-xl overflow-hidden transition-colors ${isChecked ? 'border-primary' : 'border-[#2E2E2E]'}`}>
      <button
        type="button"
        onClick={() => setExpanded((v) => !v)}
        className="w-full flex items-center gap-3 p-4 text-left"
      >
        <div className={`w-5 h-5 rounded flex items-center justify-center border flex-shrink-0 ${isChecked ? 'bg-primary border-primary' : 'border-[#3A3A3A]'}`}>
          {isChecked && <Check size={14} className="text-white" />}
        </div>
        <div className="flex-1">
          <p className="text-sm font-semibold text-[#F5F5F0]">{attribute.name}</p>
          {selectedValues.length > 0 && (
            <p className="text-xs text-[#C4C4C4] mt-0.5 truncate">{selectedValues.join(', ')}</p>
          )}
        </div>
        {expanded ? <ChevronUp size={18} className="text-[#9A9A9A]" /> : <ChevronDown size={18} className="text-[#9A9A9A]" />}
      </button>

      {expanded && (
        <div className="px-4 pb-4 border-t border-[#242424] pt-3">
          {isLoading ? (
            ...
            <>
                    ...
                    return (
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${
                          checked
                            ? 'bg-primary text-[#141414] border-primary'
                            : 'bg-[#1A1A1A] text-[#C4C4C4] border border-[#2E2E2E] hover:bg-[#242424]'
                        }`}
                      >
                        {term.name}
                      </button>
              ...
              {selectedValues.filter((v) => !terms.some((t) => t.name === v)).length > 0 && (
                <div className="flex flex-wrap gap-2 mb-3">
                  {selectedValues.filter((v) => !terms.some((t) => t.name === v)).map((v) => (
                    <span key={v} className="inline-flex items-center gap-1 bg-primary text-[#141414] text-xs px-3 py-1.5 rounded-full">
                      {v}
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 h-9 px-3 text-sm bg-[#1A1A1A] text-[#F5F5F0] border border-[#2E2E2E] rounded-lg focus:outline-none focus:border-primary"
                />
                <button
                  type="button"
                  onClick={handleAddCustom}
                  disabled={!customValue.trim()}
                  className="h-9 px-3 bg-primary text-[#141414] rounded-lg disabled:opacity-50"
                >
                  <Plus size={16} />
                </button>
              </div>
...
export default function Properties() {
  ...
  return (
    <div className="min-h-screen bg-[#0D0D0D]">
        ...
        <div className="mt-4">
          {!showNewAttr ? (
            <button
              ...
              className="w-full flex items-center justify-center gap-2 h-11 border-2 border-dashed border-[#2E2E2E] rounded-xl text-sm text-[#C4C4C4] hover:bg-[#242424] hover:border-primary"
            >
              <Plus size={16} /> Criar nova propriedade
            </button>
          ) : (
            <div className="bg-[#1A1A1A] border border-primary rounded-xl p-4">
              <p className="text-sm font-semibold text-[#F5F5F0] mb-2">Nova propriedade</p>
              <div className="flex gap-2">
                <input
                  value={newAttrName}
                  ...
                  autoFocus
                  className="flex-1 h-10 px-3 text-sm bg-[#1A1A1A] text-[#F5F5F0] border border-[#2E2E2E] rounded-lg focus:outline-none focus:border-primary"
                />
                <button
                  onClick={() => { setShowNewAttr(false); setNewAttrName('') }}
                  className="h-10 px-3 text-sm text-[#C4C4C4] border border-[#2E2E2E] rounded-lg"
                >
                  Cancelar
                </button>
                <button
                  ...
                  className="h-10 px-3 bg-primary text-[#141414] rounded-lg text-sm font-medium disabled:opacity-50"
                >
      ...
      {/* Bottom bar */}
      <div className="fixed bottom-0 left-0 right-0 bg-[#1A1A1A] border-t border-[#2E2E2E] p-4 max-w-mobile mx-auto">
