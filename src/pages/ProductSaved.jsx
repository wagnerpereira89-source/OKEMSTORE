  ...
  return (
    <Sheet open={open} onClose={onClose} title="Adicionar produtos">
      <input
        ...
        className="w-full h-10 px-3 text-sm bg-[#1A1A1A] text-[#F5F5F0] border border-[#2E2E2E] rounded-xl mb-4 focus:outline-none focus:border-primary"
      />
        ...
        {products.map((p) => {
          ...
          return (
            <button
              key={p.id}
              onClick={() => toggle(p)}
              className={`w-full flex items-center gap-3 p-2 rounded-lg transition-colors ${isSel ? 'bg-primary/10' : 'hover:bg-[#242424]'}`}
            >
              {p.images?.[0] && (
                <img src={p.images[0].src} alt="" className="w-10 h-10 rounded object-cover" />
              )}
              <span className="flex-1 text-sm text-left text-[#F5F5F0]">{p.name}</span>
    ...
    <div className="min-h-screen bg-[#0D0D0D]">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              <button
                ...
                className="p-2 rounded-lg text-[#C4C4C4] hover:bg-[#242424]"
              >
                <MoreVertical size={20} />
              </button>
              {menuOpen && (
                <div className="absolute right-0 top-full mt-1 bg-[#1A1A1A] border border-[#2E2E2E] rounded-xl shadow-lg z-10 min-w-[160px]">
                  <button
                    onClick={() => { navigator.share?.({ title: product.name, url: product.permalink }); setMenuOpen(false) }}
                    className="w-full flex items-center gap-2 px-4 py-2.5 text-sm hover:bg-[#242424]"
                  >
                  ...
                  <button
                    ...
                    className="w-full flex items-center gap-2 px-4 py-2.5 text-sm text-red-500 hover:bg-[#242424]"
                  >
      ...
      <div className="max-w-mobile mx-auto px-4 py-4 pb-32 space-y-4">
        ...
        <Card>
          ...
          {/* Alternativos (upsell) */}
          <div className="mb-4">
            <p className="text-sm font-semibold text-[#F5F5F0] mb-1">Alternativos</p>
            ...
            {product.upsell_ids?.length > 0 && (
              <p className="text-xs text-[#9A9A9A] mt-1">{product.upsell_ids.length} produto(s) adicionado(s)</p>
            )}
          </div>

          {/* Complementares (cross-sell) */}
          <div>
            <p className="text-sm font-semibold text-[#F5F5F0] mb-1">Complementares</p>
            <p className="text-sm text-[#C4C4C4] mb-2">
            ...
            {product.cross_sell_ids?.length > 0 && (
              <p className="text-xs text-[#9A9A9A] mt-1">{product.cross_sell_ids.length} produto(s) adicionado(s)</p>
            )}
          </div>
        </Card>
        ...
        <Card>
          <div className="space-y-4">
            <label className="flex items-center justify-between gap-3 cursor-pointer">
              <div className="flex items-center gap-2">
                <Truck size={16} className="text-[#C4C4C4]" />
                <span className="text-sm text-[#F5F5F0]">Esse produto possui frete grátis</span>
              </div>
              <input
                type="checkbox"
            ...
            <label className="flex items-center justify-between gap-3 cursor-pointer">
              <div className="flex items-center gap-2">
                {published ? <Eye size={16} className="text-[#C4C4C4]" /> : <EyeOff size={16} className="text-[#9A9A9A]" />}
                <span className="text-sm text-[#F5F5F0]">Exibir na minha loja</span>
              </div>
              <input
                type="checkbox"
                checked={published}
      ...
      {/* Bottom buttons */}
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-[#1A1A1A] border-t border-[#2E2E2E] flex gap-3 max-w-mobile mx-auto">
