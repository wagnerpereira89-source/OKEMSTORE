...
export default function PrintLabels() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#0D0D0D]">
        <div className="text-center">
          ...
          <p className="text-sm text-[#C4C4C4] mt-3">Carregando etiquetas...</p>
        </div>
      </div>
    )
  }
  ...
  if (orders.length === 0) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#0D0D0D]">
        <p className="text-sm text-[#C4C4C4]">Nenhum pedido encontrado.</p>
      </div>
    )
  }
    ...
    <>
      ...
      <style>{`
        @media screen {
          .print-screen-controls {
            ...
            background: #fff; border-bottom: 1px solid #2E2E2E;
      ...
      <div className="print-screen-controls">
        <button onClick={() => navigate(-1)}
          className="flex items-center gap-1 text-sm text-[#C4C4C4] hover:text-[#F5F5F0]">
          <ArrowLeft size={16} /> Voltar
        </button>
        <div className="flex-1" />
        <span className="text-xs text-[#9A9A9A]">{orders.length} {orders.length === 1 ? 'etiqueta' : 'etiquetas'}</span>
        <button onClick={() => window.print()}
          className="flex items-center gap-1 px-3 py-1.5 bg-primary text-[#141414] rounded-lg text-sm">
