...
function StockInput({ productId, variation }) {
  ...
  return (
    <input
      ...
      onBlur={handleBlur}
      className={`w-16 h-7 px-2 text-sm text-center text-[#F5F5F0] bg-[#1A1A1A] border rounded-lg outline-none transition-colors
        ${saved ? 'border-green-500' : 'border-[#2E2E2E] focus:border-primary'}`}
    />
  )
}
...
function VariationRow({ productId, variation, onEdit, onDelete }) {
  ...
  return (
    <div className="flex items-center gap-2 py-2 border-t border-[#242424] first:border-0">
      <StockInput productId={productId} variation={variation} />
      <span className="flex-1 text-sm text-[#F5F5F0] truncate">{name}</span>
...
export function ProductCard({ product, variations = [], onEditVariation, onDeleteVariation }) {
  ...
  return (
    <>
      <div className={`bg-[#1A1A1A] border border-[#2E2E2E] rounded-xl overflow-hidden ${isHidden ? 'opacity-60' : ''}`}>
        {/* Main row */}
        <div
          className="flex items-center gap-3 p-3 cursor-pointer"
          onClick={() => setExpanded((v) => !v)}
        >
          <div className="w-14 h-14 rounded-lg overflow-hidden flex-shrink-0 bg-[#242424] relative">
            {thumb ? (
          ...
          <div className="flex-1 min-w-0">
            <button
              ...
              className="text-[#F5F5F0] text-sm font-medium hover:underline text-left truncate max-w-full block"
            >
              {product.name}
            </button>
            <p className="text-xs text-[#F5F5F0] mt-0.5">
          ...
          <div className="relative" onClick={(e) => e.stopPropagation()}>
            <button
              onClick={() => setMenuOpen((v) => !v)}
              className="p-2 rounded-lg text-[#9A9A9A] hover:bg-[#242424]"
            >
            ...
            {menuOpen && (
              <>
                ...
                <div className="absolute right-0 top-full mt-1 bg-[#1A1A1A] border border-[#2E2E2E] rounded-xl shadow-lg z-20 min-w-[180px] overflow-hidden">
                  <button
                    onClick={handleEdit}
                    className="w-full flex items-center gap-2 text-left px-4 py-2.5 text-sm hover:bg-[#242424]"
                  >
                  ...
                  <button
                    ...
                    className="w-full flex items-center gap-2 text-left px-4 py-2.5 text-sm hover:bg-[#242424] disabled:opacity-50"
                  >
                    {isHidden ? <><Eye size={14} /> Mostrar no site</> : <><EyeOff size={14} /> Ocultar no site</>}
                  </button>
                  <button
                    onClick={handleDelete}
                    className="w-full flex items-center gap-2 text-left px-4 py-2.5 text-sm text-red-500 hover:bg-red-50 border-t border-[#242424]"
                  >
          ...
          <button className="p-1 text-[#9A9A9A]">
        ...
        {expanded && variations.length > 0 && (
          <div className="px-3 pb-3 border-t border-[#242424]">
            {variations.map((v) => (
              <VariationRow
                key={v.id}
        ...
        {expanded && variations.length === 0 && (
          <div className="px-3 pb-3 pt-2 border-t border-[#242424]">
            <p className="text-sm text-[#9A9A9A]">Sem variações</p>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-[#1A1A1A] rounded-2xl p-6 max-w-sm w-full">
            <div className="flex items-center gap-3 mb-3">
              ...
              <h3 className="text-base font-bold text-[#F5F5F0]">Apagar produto?</h3>
            </div>
            <p className="text-sm text-[#C4C4C4] mb-5">
              "{product.name}" será removido permanentemente. Esta ação não pode ser desfeita.
            </p>
            <div className="flex gap-2">
              <button
                onClick={() => setConfirmDelete(false)}
                className="flex-1 h-10 border border-[#2E2E2E] rounded-xl text-sm text-[#C4C4C4] hover:bg-[#242424]"
              >
                Cancelar
              </button>
              <button
