...
export default function CouponsAndShipping() {
  ...
  return (
    <div className="min-h-screen bg-[#0D0D0D]">
      {/* Header */}
      <header className="sticky top-0 z-40 bg-[#1A1A1A] border-b border-[#2E2E2E] px-4 h-20 flex items-center gap-3">
        <img
        ...
        <h1 className="text-base font-bold text-[#F5F5F0]">Cupons & Frete</h1>
      </header>

      {/* Sub-abas */}
      <div className="sticky top-20 z-30 bg-[#1A1A1A] border-b border-[#2E2E2E] px-4">
        <div className="max-w-mobile mx-auto flex gap-1">
          <button
            onClick={() => setTab('coupons')}
            className={`flex-1 py-3 text-sm font-medium border-b-2 transition-colors flex items-center justify-center gap-2 ${tab === 'coupons' ? 'border-primary text-primary' : 'border-transparent text-[#9A9A9A]'}`}>
            <Tag size={16} /> Cupons
          </button>
          <button
            onClick={() => setTab('shipping')}
            className={`flex-1 py-3 text-sm font-medium border-b-2 transition-colors flex items-center justify-center gap-2 ${tab === 'shipping' ? 'border-primary text-primary' : 'border-transparent text-[#9A9A9A]'}`}>
            <Truck size={16} /> Frete
          </button>
        </div>
      </div>
    ...
    <div className="space-y-3">
      ...
      <div className="relative">
        ...
        <input
          ...
          className="w-full h-11 pl-9 pr-3 text-sm bg-[#1A1A1A] border border-[#2E2E2E] rounded-xl focus:outline-none focus:border-primary"
        />
      </div>
      ...
      <div className="flex items-center gap-2">
        <label className="flex items-center gap-2 cursor-pointer flex-1">
          ...
          <span className="text-sm text-[#C4C4C4]">Mostrar expirados</span>
        </label>
        <button onClick={() => setEditing('new')}
          className="h-10 px-4 bg-primary text-[#141414] rounded-xl text-sm font-medium flex items-center gap-2">
          <Plus size={16} /> Novo
        </button>
      </div>
      ...
      {loading ? (
        ...
        <div className="bg-[#1A1A1A] rounded-2xl border border-[#2E2E2E] p-8 text-center">
          <Tag size={32} className="mx-auto text-[#3A3A3A] mb-2" />
          <p className="text-sm text-[#C4C4C4]">Nenhum cupom encontrado</p>
        </div>
      ) : (
...
function CouponCard({ coupon, onEdit, onDelete }) {
  ...
  return (
    <div className={`bg-[#1A1A1A] rounded-2xl border p-4 ${expired ? 'border-[#2E2E2E] opacity-60' : 'border-[#2E2E2E]'}`}>
      <div className="flex items-start justify-between gap-3 mb-2">
        ...
        <div className="flex gap-1 flex-shrink-0">
          <button onClick={onEdit} className="w-8 h-8 rounded-lg hover:bg-[#242424] flex items-center justify-center text-[#C4C4C4]">
            <Edit2 size={14} />
          </button>
...
function CouponEditor({ coupon, onSave, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40">
      <div className="bg-[#1A1A1A] w-full max-w-mobile rounded-t-2xl p-6 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold text-[#F5F5F0]">{coupon ? 'Editar cupom' : 'Novo cupom'}</h2>
          <button onClick={onClose} className="text-[#9A9A9A]"><X size={20} /></button>
        </div>

        <div className="space-y-4">
          {/* Código */}
          <div>
            <label className="text-[12px] font-medium text-[#C4C4C4] mb-1 block">Código do cupom *</label>
            <input value={code} onChange={(e) => setCode(e.target.value.toUpperCase())}
              placeholder="EX: PRIMEIRA10" disabled={!!coupon}
              className="w-full h-11 px-3 text-sm bg-[#1A1A1A] text-[#F5F5F0] border border-[#2E2E2E] rounded-xl focus:outline-none focus:border-primary disabled:bg-[#242424] disabled:text-[#9A9A9A] uppercase" />
            {coupon && <p className="text-[10px] text-[#9A9A9A] mt-1">O código não pode ser alterado depois de criado</p>}
          </div>

          {/* Tipo de desconto */}
          <div>
            <label className="text-[12px] font-medium text-[#C4C4C4] mb-1 block">Tipo de desconto</label>
            <div className="relative">
              <select value={discountType} onChange={(e) => setDT(e.target.value)}
                className="w-full h-11 px-3 pr-10 text-sm bg-[#1A1A1A] border border-[#2E2E2E] rounded-xl appearance-none focus:outline-none focus:border-primary">
                <option value="percent">% Porcentagem</option>
                <option value="fixed_cart">R$ Fixo no carrinho</option>
                <option value="fixed_product">R$ Fixo por produto</option>
              </select>
              <ChevronDown size={16} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#C4C4C4] pointer-events-none" />
            </div>
          </div>

          {/* Valor */}
          <div>
            <label className="text-[12px] font-medium text-[#C4C4C4] mb-1 block">
              Valor do desconto {discountType === 'percent' ? '(%)' : '(R$)'}
            </label>
            <input type="number" step="0.01" min="0" value={amount} onChange={(e) => setAmount(e.target.value)}
              placeholder={discountType === 'percent' ? '10' : '20,00'}
              className="w-full h-11 px-3 text-sm bg-[#1A1A1A] text-[#F5F5F0] border border-[#2E2E2E] rounded-xl focus:outline-none focus:border-primary" />
          </div>
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            <input type="checkbox" checked={freeShipping} onChange={(e) => setFree(e.target.checked)}
              className="w-4 h-4 accent-primary" />
            <span className="text-sm text-[#F5F5F0]">Conceder frete grátis</span>
          </label>
          ...
          <label className="flex items-center gap-3 cursor-pointer">
            <input type="checkbox" checked={individualUse} onChange={(e) => setIndiv(e.target.checked)}
              className="w-4 h-4 accent-primary" />
            <span className="text-sm text-[#F5F5F0]">Uso individual (não combina com outros cupons)</span>
          </label>

          {/* Valor mínimo */}
          <div>
            <label className="text-[12px] font-medium text-[#C4C4C4] mb-1 block">Valor mínimo do pedido (R$)</label>
          ...
          {/* Limite de uso */}
          <div>
            <label className="text-[12px] font-medium text-[#C4C4C4] mb-1 block">Limite total de usos</label>
          ...
          {/* Validade */}
          <div>
            <label className="text-[12px] font-medium text-[#C4C4C4] mb-1 block">Data de expiração</label>
        ...
        <div className="flex gap-3 mt-6">
          <button onClick={onClose} disabled={saving}
            className="flex-1 h-11 border border-[#2E2E2E] rounded-xl text-sm text-[#C4C4C4]">
            Cancelar
          </button>
          <button onClick={handleSubmit} disabled={saving}
            className="flex-1 h-11 bg-primary text-[#141414] rounded-xl text-sm font-medium flex items-center justify-center gap-2 disabled:opacity-60">
...
function ShippingTab() {
  ...
  return (
    <div className="space-y-4">
      {/* Filtro de período */}
      <div className="bg-[#1A1A1A] rounded-2xl border border-[#2E2E2E] p-4 flex items-center justify-between">
        <div>
          <p className="text-sm font-medium text-[#F5F5F0]">{periodDates.label}</p>
          <p className="text-xs text-[#9A9A9A] mt-0.5">{orders.length} pedidos analisados</p>
        </div>
        <button onClick={() => setFO(true)}
          className="h-9 w-9 bg-[#242424] rounded-xl flex items-center justify-center text-[#C4C4C4]">
          <Calendar size={16} />
        </button>
      </div>
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-[#1A1A1A] rounded-2xl border border-[#2E2E2E] p-4">
              <p className="text-xs text-[#9A9A9A] mb-1">Total em frete</p>
              <p className="text-lg font-bold text-[#F5F5F0]">{formatBRL(shippingByMethod.total)}</p>
            </div>
            <div className="bg-[#1A1A1A] rounded-2xl border border-[#2E2E2E] p-4">
              <p className="text-xs text-[#9A9A9A] mb-1">Frete médio</p>
              <p className="text-lg font-bold text-[#F5F5F0]">{formatBRL(shippingByMethod.avg)}</p>
            </div>
          </div>

          {/* Lista por método */}
          <div className="bg-[#1A1A1A] rounded-2xl border border-[#2E2E2E] p-4">
            <p className="text-base font-bold text-[#F5F5F0] mb-1">Por método de envio</p>
            <p className="text-xs text-[#9A9A9A] mb-4">{periodDates.label}</p>
            {shippingByMethod.list.length === 0 ? (
              <p className="text-sm text-[#9A9A9A] text-center py-4">Nenhum frete neste período</p>
            ) : (
              <div className="space-y-4">
                  ...
                  return (
                    <div key={m.method}>
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-sm font-medium text-[#F5F5F0] truncate max-w-[200px]">{m.method}</span>
                        <div className="text-right">
                          <span className="text-sm font-bold text-[#F5F5F0]">{formatBRL(m.total)}</span>
                          <p className="text-xs text-[#9A9A9A]">{m.count}x · {pct.toFixed(1)}%</p>
                        </div>
                      </div>
...
function PeriodModal({ period, customFrom, customTo, onSave, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40">
      <div className="bg-[#1A1A1A] w-full max-w-mobile rounded-t-2xl p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold text-[#F5F5F0]">Período</h2>
          <button onClick={onClose} className="text-[#9A9A9A]"><X size={20} /></button>
        </div>

        <div className="relative mb-4">
          <select value={p} onChange={(e) => setP(e.target.value)}
            className="w-full h-11 px-3 pr-10 text-sm bg-[#1A1A1A] border-2 border-primary rounded-xl appearance-none focus:outline-none">
        ...
        {p === 'custom' && (
          <div className="grid grid-cols-2 gap-3 mb-4">
            <div>
              <label className="text-[12px] font-medium text-[#C4C4C4] mb-1 block">De</label>
              <input type="date" value={from} onChange={(e) => setFrom(e.target.value)}
                className="w-full h-11 px-3 text-sm bg-[#1A1A1A] text-[#F5F5F0] border border-[#2E2E2E] rounded-xl focus:outline-none focus:border-primary" />
            </div>
            <div>
              <label className="text-[12px] font-medium text-[#C4C4C4] mb-1 block">Até</label>
        ...
        <div className="flex gap-3">
          ...
          <button onClick={handleSave}
            className="flex-1 h-11 bg-primary text-[#141414] rounded-xl text-sm font-medium">
            Salvar
          </button>
        </div>
      </div>
