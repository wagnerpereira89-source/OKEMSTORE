    ...
    <div>
      ...
      <div className="flex flex-wrap gap-2 mb-2">
        {selected.map((cat) => (
          ...
          >
            {cat.name}
            <button onClick={() => removeSelected(cat.id)} className="text-[#9A9A9A] hover:text-red-500">
              <X size={14} />
            </button>
          </span>
        ))}
      ...
      <Sheet open={open} onClose={handleClose} title="Categorias">
        ...
        <form onSubmit={handleCreate} className="flex gap-2 mb-4">
          ...
          <button
            ...
            className="h-9 px-3 bg-primary text-[#141414] rounded-lg text-sm disabled:opacity-50"
          >
            <Plus size={16} />
          </button>
        </form>
            ...
            {sortCategoriesHierarchical(categories).map((cat) => {
              ...
              return (
                <button
                  ...
                  type="button"
                  onClick={() => toggle(cat)}
                  className="w-full flex items-center justify-between px-3 py-2.5 rounded-lg hover:bg-[#242424] transition-colors"
                  style={isChild ? { paddingLeft: '28px' } : undefined}
                >
                  <span className={`text-sm text-[#F5F5F0] ${isChild ? 'font-normal' : 'font-bold'}`}>
                    {cat.name}
                  </span>
        ...
        <div className="pt-4 border-t border-[#2E2E2E] mt-4">
