    ...
    <div className="min-h-screen bg-[#0D0D0D]">
      ...
      <div className="max-w-mobile mx-auto px-4 py-4 space-y-4">
        ...
        <div className="bg-[#1A1A1A] border border-[#2E2E2E] rounded-xl p-4">
          <label className="text-[13px] font-medium text-[#C4C4C4] block mb-2">Propriedade</label>
          <select
            value={selectedAttrId || (showCustomAttrInput ? '__new__' : '')}
            onChange={handleAttrChange}
            className="w-full h-10 px-3 text-sm border border-[#2E2E2E] rounded-lg focus:outline-none focus:border-primary bg-[#1A1A1A]"
          >
          ...
          {showCustomAttrInput && (
            <input
              ...
              className="mt-2 w-full h-10 px-3 text-sm bg-[#1A1A1A] text-[#F5F5F0] border border-[#2E2E2E] rounded-lg focus:outline-none focus:border-primary"
            />
          )}
        </div>
        ...
        {(selectedAttrId || showCustomAttrInput) && (
          <div className="bg-[#1A1A1A] border border-[#2E2E2E] rounded-xl p-4 space-y-4">
            {/* Selected values */}
            <div>
              <p className="text-[13px] font-medium text-[#C4C4C4] mb-2">Valores selecionados</p>
              {selectedValues.length === 0 ? (
                <p className="text-sm text-[#9A9A9A]">Nenhum valor selecionado ainda</p>
              ) : (
                <div className="flex flex-wrap gap-2">
                  {selectedValues.map((val) => (
                    <span
                      key={val}
                      className="inline-flex items-center gap-1 bg-primary text-[#141414] text-sm px-3 py-1 rounded-full"
                    >
                      {val}
            ...
            {/* Custom value input */}
            <div>
              <p className="text-[13px] font-medium text-[#C4C4C4] mb-2 flex items-center gap-1">
                <Plus size={14} />
            ...
            {terms.length > 0 && (
              <div>
                <p className="text-[13px] font-medium text-[#C4C4C4] mb-2">Valores sugeridos</p>
                  ...
                  {terms.map((term) => {
                    ...
                    return (
                      <button
                        ...
                        className="w-full flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-[#242424] transition-colors"
                      >
                        <div className={`w-5 h-5 rounded flex items-center justify-center border ${checked ? 'bg-primary border-primary' : 'border-[#2E2E2E]'}`}>
                          {checked && <Check size={12} className="text-white" />}
                        </div>
                        <span className="text-sm text-[#F5F5F0]">{term.name}</span>
                      </button>
                    )
                  })}
                </div>
