          ...
          {data.map((d, i) => {
            ...
            return (
              <g key={i}>
                ...
                {showLabel && (
                  <text x={x + barW / 2} y={chartH + 16} textAnchor="middle" fontSize="9" fill="#9A9A9A">
                    {getDayLabel(d.date)}
                  </text>
                )}
              </g>
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40">
      <div className="bg-[#1A1A1A] w-full max-w-mobile rounded-t-2xl p-6">
        <div className="flex items-center justify-between mb-1">
          <h2 className="text-xl font-bold text-[#F5F5F0]">Ajustar períodos</h2>
          <button onClick={onClose} className="text-[#9A9A9A]"><X size={20} /></button>
        </div>
        <p className="text-sm text-[#C4C4C4] mb-5">Selecione um período principal para visualizar os dados.</p>

        <label className="text-[12px] font-medium text-[#C4C4C4] mb-1 block">Período principal</label>
        ...
        <label className="flex items-center gap-3 mb-6 cursor-pointer">
          ...
          <span className="text-sm text-[#F5F5F0]">Comparar com períodos anteriores</span>
        </label>
  ...
  return (
    <div className="min-h-screen bg-[#0D0D0D] overflow-x-hidden">
      ...
      <header className="sticky top-0 z-40 bg-[#1A1A1A] border-b border-[#2E2E2E] px-4 h-20 flex items-center gap-3">
        ...
        <h1 className="text-base font-bold text-[#F5F5F0] truncate min-w-0">Dashboard</h1>
        <div className="flex-1" />
        {notifSupported && (
          <button onClick={handleToggleNotifications}
            disabled={notifLoading}
            className={`p-2 rounded-lg transition-colors ${notifEnabled ? 'text-primary bg-primary/10' : 'text-[#C4C4C4] hover:bg-[#242424]'} disabled:opacity-50`}
        ...
        <button onClick={load} disabled={loading}
          className="p-2 rounded-lg text-[#C4C4C4] hover:bg-[#242424] disabled:opacity-40">
      ...
      <div className="max-w-mobile mx-auto px-4 pb-28 space-y-4 py-4">
        ...
        {/* Última atualização */}
        {lastUpdate && (
          <p className="text-xs text-[#9A9A9A] flex items-center gap-1">
        ...
        <div className="space-y-3">
          {metrics.map((m) => {
            ...
            return (
              <div key={m.key}
                className={`bg-[#1A1A1A] rounded-2xl border-2 p-4 flex items-start justify-between transition-colors cursor-pointer ${isSelected ? 'border-primary' : 'border-[#2E2E2E]'}`}
                onClick={() => setMetric(m.key)}
              >
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="w-3 h-3 rounded-sm inline-block" style={{ background: metricColors[m.key] }} />
                    <span className="text-sm text-[#C4C4C4]">{m.label}</span>
                  </div>
                  <p className="text-xl font-bold text-[#F5F5F0]">{loading ? '—' : m.format(m.value)}</p>
                  {compare && !loading && change !== null && (
                    <ChangeTag value={change} />
                  )}
                </div>
                <div className={`w-5 h-5 rounded-full border-2 flex-shrink-0 mt-1 ${isSelected ? 'border-primary bg-primary' : 'border-[#3A3A3A]'}`}>
                  {isSelected && <div className="w-full h-full rounded-full bg-[#1A1A1A] scale-50 block" />}
                </div>
              </div>
            )
          })}
        ...
        {!loading && paymentData.length > 0 && (
          <div className="bg-[#1A1A1A] rounded-2xl border border-[#2E2E2E] p-4">
            <p className="text-base font-bold text-[#F5F5F0] mb-4">Métodos de pagamento</p>
              ...
              {paymentData.map((d, i) => (
                <div key={d.name} className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm text-[#F5F5F0]">{d.name}</span>
                  </div>
                  <span className="text-sm text-[#C4C4C4]">{d.pct}% ({d.count})</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {!loading && topProducts.length > 0 && (
          <div className="bg-[#1A1A1A] rounded-2xl border border-[#2E2E2E] p-4">
            <div className="flex items-center justify-between mb-1">
              <p className="text-base font-bold text-[#F5F5F0]">Melhores desempenhos</p>
            </div>
            <p className="text-xs text-[#9A9A9A] mb-4">{periodDates.label}</p>
            <div className="flex justify-between text-xs font-semibold text-[#9A9A9A] mb-2 px-1">
              <span>Produtos</span>
              ...
              {topProducts.map((p, i) => (
                <div key={i} className="flex items-center gap-3 py-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-[#F5F5F0] truncate">{p.name}</p>
                    <p className="text-xs text-[#C4C4C4]">Vendas líquidas: {formatBRL(p.total)}</p>
                  </div>
                  <span className="text-base font-bold text-[#F5F5F0]">{p.qty}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {!loading && topPickup.length > 0 && (
          <div className="bg-[#1A1A1A] rounded-2xl border border-[#2E2E2E] p-4">
            <p className="text-base font-bold text-[#F5F5F0]">Top Unidades de Retirada</p>
            <p className="text-xs text-[#9A9A9A] mb-4">Ranking de lojas físicas</p>
            <div className="space-y-4">
              {topPickup.map((u, i) => (
                <div key={i}>
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-2">
                      <span className="text-xs text-[#9A9A9A] w-6">#{i + 1}</span>
                      <span className="text-sm font-medium text-[#F5F5F0] truncate max-w-[180px]">{u.name}</span>
                    </div>
                    <div className="text-right">
                      <span className="text-sm font-bold text-[#F5F5F0]">{u.count}x</span>
                      <p className="text-xs text-[#9A9A9A]">{formatBRL(u.total)}</p>
                    </div>
                  </div>
        ...
        {!loading && topCities.length > 0 && (
          <div className="bg-[#1A1A1A] rounded-2xl border border-[#2E2E2E] p-4">
            <p className="text-base font-bold text-[#F5F5F0] mb-4">Top 10 cidades</p>
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-[10px] font-semibold text-[#9A9A9A] uppercase">
                ...
                <tbody className="divide-y divide-[#242424]">
                  {topCities.map((c, i) => (
                    <tr key={i}>
                      <td className="py-2 text-[#9A9A9A] text-xs">{i + 1}</td>
                      <td className="py-2 font-medium text-[#F5F5F0]">{c.city}</td>
                      <td className="py-2 text-center text-[#C4C4C4]">{c.state}</td>
                      <td className="py-2 text-center text-[#C4C4C4]">{c.orders}</td>
                      <td className="py-2 text-right text-[#C4C4C4]">{formatBRL(c.total)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            {lastUpdate && (
              <p className="text-xs text-[#9A9A9A] text-right mt-3">
        ...
        {loading && (
          <div className="flex flex-col items-center py-12 gap-3">
            ...
            <p className="text-sm text-[#9A9A9A]">Carregando dados...</p>
          </div>
        )}
      </div>
