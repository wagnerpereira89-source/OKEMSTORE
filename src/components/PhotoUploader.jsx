...
function SortablePhoto({ photo, index, onSetMain, onRemove }) {
  ...
  return (
    <div
      ...
      {...attributes}
      {...listeners}
      className={`relative aspect-square rounded-lg overflow-hidden bg-[#242424] ${
            ...
            >
              <div className="grid grid-cols-3 gap-2 mb-3">
                ...
                <button
                  type="button"
                  onClick={() => inputRef.current?.click()}
                  className="aspect-square rounded-lg border-2 border-dashed border-[#2E2E2E] flex items-center justify-center text-[#9A9A9A] hover:border-action hover:text-action transition-colors"
                >
                  <Plus size={20} />
                </button>
              </div>
